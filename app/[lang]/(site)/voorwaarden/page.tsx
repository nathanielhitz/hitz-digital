...
export async function generateMetadata({ params }: LangParams): Promise<Metadata> {
  ...
  return pageMetadata(lang, "voorwaarden", getDict(lang).pages.voorwaarden.meta);
}

export default async function VoorwaardenPage({ params }: LangParams) {
  const lang = langOf((await params).lang);
  const { pages, legal } = getDict(lang);
  const t = pages.voorwaarden;
  const Body = legal.TermsBody;
  return (
    <main id="main" className="relative z-[2] bg-deep">
      ...
      <section className="px-[clamp(20px,5vw,64px)] pb-10 md:pb-12">
        <Container>
          <Prose>
            <p className="text-[13.5px] text-faint">{t.updatedLine(t.updated)}</p>
            <Body />
          </Prose>
        </Container>
      </section>
